...
- `InputController` uses capture-phase Pointer Events but consumes an event only after an enabled toolbar mode or the Ctrl override claims a primary mouse gesture. Cancelling that pointerdown suppresses Zotero's compatibility mousedown — the suppression only holds while the pointer is not captured, so `setPointerCapture()` is deferred to the first pointermove (with a zero-delay fallback), which keeps claimed gestures out of Zotero's mousedown-driven selection handling in the common case while edge drags still release. While a gesture is claimed it additionally blocks text selection in the nested viewer (scoped `user-select: none !important` stylesheet, capture-phase `selectstart` cancel, and a `selectionchange` clear for programmatic selection); user testing shows a large reduction with a small, accepted residual in some cases. OFF-mode plain drag, Alt, and Ctrl+Alt are untouched, and everything is released through the single `finishGesture()` chokepoint. Pen arbitration remains a v0.2 extension point.
